...
{
	/// <summary>
	/// This the main chess game class. It contains a chess board and two players.
    /// Also initializes and maintains the status of the game.
	/// </summary>
    [Serializable]
	public class Game
	{
        ...
        public static Side.SideType GameTurn;		    // Current game turn i.e. White or Black

        private Stack TotalMovesHistory;		// Contains all moves made by the user		
        private Stack m_RedoMovesHistory;	// Contains all the Redo moves made by the user
		private Rules PlayerRules;			    // Contains all the chess rules
		...
		public Game()
		{
			...
			PlayerRules = new Rules(Board, this);	
			TotalMovesHistory = new Stack();
			m_RedoMovesHistory = new Stack();
            m_WhitePlayer = new Player(new Side(Side.SideType.White), Player.Type.Human, PlayerRules);	// For the start both player are human
            m_BlackPlayer = new Player(new Side(Side.SideType.Black), Player.Type.Human, PlayerRules);	// For the start both player are human
		}
        ...
        public XmlNode XmlSerialize(XmlDocument xmlDoc)
        {
            ...
            object[] moves = TotalMovesHistory.ToArray();
        ...
        public void XmlDeserialize(XmlNode xmlGame)
        {
            ...
            m_WhitePlayer.GameRules = PlayerRules;
            ...
            m_BlackPlayer.GameRules = PlayerRules;
		...
		{
			return PlayerRules.GetLegalMoves(source);
		}
		...
		public int DoMove(string source, string dest)
		{
            ...
            if (this.Board[source].piece != null && this.Board[source].piece.Type != Piece.PieceType.Empty && this.Board[source].piece.Side.type == GameTurn)
			{
				...
				MoveResult=PlayerRules.DoMove(UserMove);

				// If the move was successfully executed
				if (MoveResult==0)
				{
					TotalMovesHistory.Push(UserMove);
					NextPlayerTurn();
				}
			}
			else
		...
		public bool UnDoMove()
		{
            ...
            if (TotalMovesHistory.Count>0)
			{
                Board.maxMoves += 0.5;
                Move UserMove = (Move)TotalMovesHistory.Pop();	// Ge the user move from his moves history stack
				m_RedoMovesHistory.Push(UserMove);			// Add this move in user Redo moves stack
				PlayerRules.UndoMove(UserMove);					// Undo the user move
		...
		public bool ReDoMove()
		{
			...
			if (m_RedoMovesHistory.Count>0)
			{
				...
				TotalMovesHistory.Push(UserMove);				// Add to the user undo move list
				PlayerRules.DoMove(UserMove);					// Undo the user move
				NextPlayerTurn();							// Switch the user turn
				return true;
			}
			else
		...
		{
			return PlayerRules.IsStaleMate(PlayerSide);
		}

		// Return true if the current player is under check
		public bool IsUnderCheck()
		{
			return PlayerRules.IsUnderCheck(GameTurn);
		}
		...
		public Move GetLastMove()
		{
			...
			if (TotalMovesHistory.Count>0)
			{
				return (Move)TotalMovesHistory.Peek();	// Ge the user move from his moves history stack
			}
			return null;
		}
		...
		public void SetPromoPiece(Piece PromoPiece)
		{
			...
			if (TotalMovesHistory.Count>0)
			{
				Move move=(Move)TotalMovesHistory.Peek();	// Ge the user move from his moves history
